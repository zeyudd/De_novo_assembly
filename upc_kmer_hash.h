...
size_t lookup_kmer(shared [KMER_PACKED_LENGTH] char *kmer_key, shared [1] kmer_t *kmer_aux, 
                    shared [LOAD_FACTOR] bucket_t *hashtable, size_t hashlen, const unsigned char *kmer)
{
    ...
    for (; result!=hashlen; ) {
        upc_memget(packed_kmer_buf, kmer_key + result * KMER_PACKED_LENGTH, KMER_PACKED_LENGTH);
        if ( memcmp(packedKmer, packed_kmer_buf, KMER_PACKED_LENGTH * sizeof(char)) == 0 ) {
            return result;
        }
        result = kmer_aux[result].next;
   }
   return hashlen;
}

/* Adds a kmer and its extensions in the hash table (note that a memory heap should be preallocated. ) */
int add_kmer(shared kmer_t *kmer_aux, shared [KMER_PACKED_LENGTH] char *kmer_key, size_t *posInHeap, 
             shared bucket_t *hashtable, size_t hashlen, const unsigned char *kmer_to_add, char left_ext, char right_ext)
{
   /* Pack a k-mer sequence appropriately */
    ...
    upc_memput(kmer_key + pos * KMER_PACKED_LENGTH, packedKmer, KMER_PACKED_LENGTH * sizeof(char));

    kmer_aux[pos].l_ext = left_ext;
    kmer_aux[pos].r_ext = right_ext;
    /* Fix the next pointer to point to the appropriate kmer struct */
    kmer_aux[pos].next = hashtable[hashval].head;
    ...
    unpackSequence((unsigned char *)&kmer_key[pos * KMER_PACKED_LENGTH], unpackedKmer, KMER_LENGTH);
...
void addKmerToStartList(shared kmer_t *kmer_aux, size_t posInHeap, start_kmer_t **startKmersList)
{
